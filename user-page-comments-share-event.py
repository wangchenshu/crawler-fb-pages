...
if len(sys.argv) > 1:
    ...
    out_file = 'who_share_comments_{0}.csv'.format(what_page_id)
    ...
    while 'paging' in res.json(): 
        for information in res.json()['data']:
            if 'message' in information:
                fb_id = information['from']['id']
                fb_name = '"%s"' % (information['from']['name'])
                fb_user_message = '"%s"' % (information['message'])
                message_created_time = '"{0}"'.format(parse(information['created_time']))
                event_page = '"http://www.facebook.com/{0}"'.format(what_page_id)
                information_list.append([fb_id, fb_name, fb_user_message.replace('\n',' '), message_created_time, event_page])
    ...
    information_df = pd.DataFrame(information_list, columns=['fb_id', 'fb_name', 'message', 'message_created_time', 'event_page'])
