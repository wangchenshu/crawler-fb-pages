...
if len(sys.argv) > 1:
    ...
    while 'paging' in res.json(): 
        for information in res.json()['data']:
            if 'message' in information:
                ...
                information_list.append([i, what_page, fb_user_link, fb_user_name, fb_user_message, created_time])
                i += 1
    ...
    information_df = pd.DataFrame(information_list, columns=['序號', '粉絲專頁', '臉書連結', '姓名', '留言內容', '留言時間']) 
